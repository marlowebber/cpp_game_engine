...
void growInto(unsigned int from, unsigned int to, unsigned int organ, bool fromSeed)
{
	if (!( materialBlocksMovement (game.world[to].wall)))
	{
		if (organ == MATERIAL_SEED || MATERIAL_POLLEN) // production of plant gametes
		{


			if (organ == MATERIAL_SEED)
			{
				game.world[to].seedIdentity = getNewPlantIdentity();
			}
		...
		else                                          // production of functional tissues
		{
			...
			if (organ == MATERIAL_WOOD || organ == MATERIAL_LEAF)
			{
	...
	while (true)
	{
		...
		else
		{
			...
			case PLANTGENE_BRANCH:
			{
				...
				// skipAhead = game.world[worldI].plantGenes[nextGene];
				// skip = true;
			...
			case PLANTGENE_BUD:
			{
				for (int i = 0; i < nNeighbours; ++i)
				{
					if (game.world[worldI].growthMatrix[i])
					{
						...
						if (neighbour < worldSquareSize)
						{
							growInto(worldI, neighbour, MATERIAL_BUD, false);
							// if (skip) { game.world[neighbour].geneCursor += skipAhead; skip = false;   }
							if (branch) {  clearGrowthMask(neighbour) ;}
						}
					}
				}
				done = true;
			...
			case PLANTGENE_LEAF:
			{
				for (int i = 0; i < nNeighbours; ++i)
				{
					if (game.world[worldI].growthMatrix[i])
					{
						...
						if (neighbour < worldSquareSize)
						{
							growInto(worldI, neighbour, MATERIAL_LEAF, false);
							// if (skip) { game.world[neighbour].geneCursor += skipAhead; skip = false; }
							if (branch) {  clearGrowthMask(neighbour) ;}
						}
					}
			...
			case PLANTGENE_WOOD:
			{
				for (int i = 0; i < nNeighbours; ++i)
				{
					if (game.world[worldI].growthMatrix[i])
					{
						...
						if (neighbour < worldSquareSize)
						{
							growInto(worldI, neighbour, MATERIAL_WOOD, false);
							// if (skip) { game.world[neighbour].geneCursor += skipAhead; skip = false; }
							if (branch) {  clearGrowthMask(neighbour) ;}
						}
					}
			...
			// 	if (skipAhead < nextGene) { skipAhead = nextGene; }  // otherwise individual plants can will get into loops and grow enormously large. it's a good but boring strategy.
			...
			case PLANTGENE_SEEDCOLOR_RED:
			{
				...
				game.world[worldI].seedColor = clampColor(game.world[worldI].grassColor);
				break;
			}
						...
						for (int n = 0; n < nNeighbours; ++n)
						{
							...
							if (neighbour < worldSquareSize)
							{
								...
								if (game.world[neighbour].seedState == MATERIAL_NOTHING)
								{
									...
									// game.world[neighbour].seedState = MATERIAL_POLLEN;
									growInto(worldI, neighbour, MATERIAL_POLLEN, false);
									break;
								}
							}
						}
						...
						if (game.world[neighbour].seedState == MATERIAL_POLLEN)
						{
							if (game.world[neighbour].seedIdentity != game.world[worldI].plantIdentity)
							{
								...
								if ( totalDifference < plantSpeciesThreshold)
								{
									...
									growInto(worldI, neighbour, MATERIAL_SEED, false);
									break;
								}
							}
						}
					...
					for (int k = 0; k < numberOfSpecies; ++k)// spawn lots of the example animal
					{
						...
						if (game.speciesPopulationCounts[k] == 0 && k != 0)
						{
							...
							for (int ee = 0; ee < 32; ++ee)
							{
								...
								int whatToSpawn = extremelyFastNumberFromZeroTo(2);
